describe('Ext.grid.plugin.Exporter', function() {
    ...
    describe('exporters', function() {
        ...
        it('should generate an xlsx cell correctly', function () {
            ...
            expect(excel.getWorkbook().getSharedStrings().getStrings()[0]).toBe('test');
            ...
            expect(cell.render()).toBe('<c r="F1" t="d"><v>' + Ext.Date.format(d, 'Y-m-d\\TH:i:s.u') + '</v></c>');

            cell = row.addCell({
                value: false
            });
            expect(cell.render()).toBe('<c r="G1" t="b"><v>false</v></c>');

            cell = row.addCell({});
            expect(cell.render()).toBe('<c r="H1"/>');
        });
    });
